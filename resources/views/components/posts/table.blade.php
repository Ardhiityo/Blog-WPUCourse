@session('success')
    <div class="flex justify-center">
        <div id="toast-success"
            class="flex items-center absolute top-24 w-full max-w-xs p-4 mb-4 text-gray-500 bg-white rounded-lg shadow-sm dark:text-gray-400 dark:bg-gray-800"
            role="alert">
            <div
...
<section class="sm:p-5 antialiased">
    <div class="mx-auto max-w-screen-xl">
        <!-- Start coding here -->
        <div>
            <h6 class="text-lg font-medium text-gray-500 mb-3 dark:text-white">Posts By : {{ Auth::user()->name }}</h6>
        </div>
        <div class="bg-white dark:bg-gray-800 relative border sm:rounded-lg overflow-hidden">
            <div class="flex flex-col md:flex-row items-center space-y-3 md:space-y-0 md:space-x-4 p-4">
                <div class="w-full md:w-1/2">
                    <form class="flex items-center" action="{{ route('dashboard.index') }}" method="get">
                ...
                <div
                    ...
                    <a href="{{ route('dashboard.create') }}" type="button" id="createProductModalButton"
                    ...
                    <tbody>
                        @foreach ($posts as $post)
                            <tr class="border-b dark:border-gray-700">
                                <td class="px-4 py-3">{{ $posts->firstItem() + $loop->index }}</td>
                                <td class="px-4 py-3">{{ $post->title }}</td>
                                <td class="px-4 py-3">{{ $post->category->name }}</td>
                                <td class="px-4 py-3">{{ $post->created_at->diffForHumans() }}</td>
                                <td class="px-4 py-3 flex items-center justify-end">
                                    <button id="posts-{{ $post->id }}-dropdown-button"
                                        data-dropdown-toggle="posts-{{ $post->id }}-dropdown"
                                    ...
                                    <div id="posts-{{ $post->id }}-dropdown"
                                        class="hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
                                        <ul class="py-1 text-sm"
                                            aria-labelledby="posts-{{ $post->id }}-dropdown-button">
                                            <li>
                                            ...
                                            <li>
                                                <a href="{{ route('dashboard.show', ['post' => $post->id]) }}"
                                                    type="button" data-modal-target="readProductModal"
